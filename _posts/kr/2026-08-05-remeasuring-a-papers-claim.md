...
다만 개인끼리가 아니라 **그룹끼리의 일치도**입니다. 그리고 논문을 뒤져도 **아홉 명을 어떻게 두 그룹으로 나눴는지, 그룹 안에서 점수를 어떻게 합쳤는지가 안 나옵니다.** 평균을 냈다면 개인의 잡음이 상쇄돼 값이 높아지고, 78건을 나눠 봤다면 겹치는 부분에서만 계산했을 텐데 그 개수를 알 수 없습니다. **합치는 방식에 따라 이 값을 상한선으로 쓸 수 있는지가 갈립니다.** 그래서 기준선으로는 쓰되 확정된 상한선으로 두지는 않았습니다.
